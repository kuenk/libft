...
//	ft_strlen
	...
	str= "\n";
	printf("El str es un '\'n  y su longitud es %zu caracteres\n", ft_strlen(str));
	printf("\n");
...
//	ft_strnstr
	...
	//printf("Subcadena encontrada en la posicion %ld.\n", strnstr(haystack, needle, total_len) - haystack);
	printf("\n");

//	ft_atoi
	printf("ft_atoi\n");
	printf("Esta funcion convierte un str a un int.\n");
	const char *str_atoi = "2703199165498754";
...
//	ft_substr
	...
	char *substring1 = ft_substr(input_substr, 30, 5);
...
/*	ft_strmapi
	...
	result_mapi = ft_strmapi(original_mapi, foo);
	if (result_mapi != NULL)
 {
		printf("Str original: %s\n", original_mapi);
...
//	ft_lstlast
	...
	node1->content = (void *)(intptr_t)1;
	node2->content = (void *)(intptr_t)2;
	node3->content = (void *)(intptr_t)3;
	node1->next = node2;
	node2->next = node3;
	node3->next = NULL;
...
//	ft_lstmap
	...
	*(int*)node_map3->content = 3;
	...
	node_map3->next = NULL;
	t_list *new_list = ft_lstmap(node_map1, foo, free);
